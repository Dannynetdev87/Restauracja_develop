<x-app>
    <x-slot:title>Aktualne zamówienie baru - SmakPrzeszłości</x-slot>
    ...
    <section class="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <span class="text-sm font-bold uppercase text-brand-accent">Panel baru</span>
                <h1 class="mt-2 text-3xl font-black text-brand-dark">Aktualne zamówienie</h1>
                <p class="mt-2 max-w-3xl text-sm text-brand-accent">
                    Widok pokazuje najstarsze aktywne zamówienie z napojami. Pełną kolejkę znajdziesz w dashboardzie.
                </p>
            </div>
                            ...
                            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                ...
                                <div class="w-full md:w-56">
                                    @if($item->status === \App\Models\OrderItem::STATUS_NEW)
                                        <form method="POST" action="{{ route('bar.order-items.status', $item) }}">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="status" value="{{ \App\Models\OrderItem::STATUS_PREPARING }}">
                                            <input type="hidden" name="redirect_to" value="bar.current">
                                    ...
                                    @if(in_array($item->status, [\App\Models\OrderItem::STATUS_NEW, \App\Models\OrderItem::STATUS_PREPARING], true))
                                        <form method="POST" action="{{ route('bar.order-items.cancel', $item) }}" class="mt-2">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="redirect_to" value="bar.current">
        ...
        @else
            <div class="rounded-lg border border-brand-dark/15 bg-white px-6 py-12 text-center shadow-sm">
                <h2 class="text-2xl font-black text-brand-dark">Brak aktywnych pozycji baru</h2>
                <p class="mt-2 text-sm text-brand-accent">Nowe napoje pojawią się tutaj automatycznie po dodaniu pozycji przez kelnera.</p>
            </div>
        @endif
    </section>
</x-app>
