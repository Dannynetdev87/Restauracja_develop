                        ...
                        <section class="rounded-lg border border-brand-dark/15 bg-brand-card p-5">
                            ...
                            <div class="space-y-4">
                                @forelse($column['items'] as $item)
                                    @php
                                        $openedAt = $item->order->opened_at;
                                        $waitingMinutes = $openedAt ? (int) round($openedAt->diffInMinutes(now())) : 0;
                                        ...
                                        <div class="flex items-start justify-between gap-4">
                                            <div>
                                                ...
                                                <p class="mt-1 text-sm text-brand-accent">
                                                    Stolik {{ $item->order->table->number }} · od {{ $openedAt?->format('H:i') ?? '--:--' }}
                                                </p>
                                                <span class="mt-2 inline-flex rounded-md px-2 py-1 text-xs font-bold {{ $isDelayed ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700' }}">
                                                    Czeka {{ $waitingMinutes }} min
                                                </span>
                                            </div>
                    ...
                    <div class="space-y-3">
                        @forelse($queueOrders as $orderItems)
                            @php
                                ...
                                $waitingMinutes = $order->opened_at ? (int) round($order->opened_at->diffInMinutes(now())) : 0;
                            ...
                            <div class="rounded-lg border border-brand-dark/10 bg-white p-4 shadow-sm">
                                ...
                                <div class="mt-2 flex flex-wrap items-center justify-between gap-2 text-sm text-brand-accent">
                                    <span>Stolik {{ $order->table->number }} · od {{ $order->opened_at?->format('H:i') ?? '--:--' }}</span>
                                    <span class="font-bold {{ $isDelayed ? 'text-red-700' : 'text-brand-accent' }}">Czeka {{ $waitingMinutes }} min</span>
                                </div>
                            </div>
                        @empty
                            <div class="rounded-lg border border-dashed border-brand-dark/20 bg-white px-4 py-8 text-center text-sm font-semibold text-brand-accent">
